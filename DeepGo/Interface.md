...
Подробнее про то, когда переменная уходит в кучу, а когда остаётся на стеке — разберём отдельно в теме про [[Allocators#Object Allocation: Escape Analysis|аллокаторы и escape analysis]].
...
Мы уже разбирали это со стороны структур в [[Structs#Как методы устроены под капотом|Structs]] — тут та же идея, но со стороны интерфейсов. Хороший пример, который наглядно это показывает — метод можно вызвать как обычную функцию, передав receiver первым аргументом:
