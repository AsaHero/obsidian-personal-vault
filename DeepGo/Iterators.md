...
`Seq` — итератор с одним значением на элемент, `Seq2` — с парой (обычно ключ/значение или индекс/значение). Произносится `Seq` как "seek" — от "sequence".
...
> ⚠️ Порядок вывода при итерации по `map` не гарантирован — при разных запусках `"pi"` и `"e"` могут выводиться в разном порядке. Это давнее и осознанное свойство самого языка, итераторы над картами тут ничего не меняют.
