...
На базовом уровне пакет `reflect` — это просто механизм для изучения пары **(тип, значение)**, которая хранится внутри интерфейсной переменной. Помнишь из темы про [[Interface|интерфейсы]], что [[Interface#Интерфейс под капотом|интерфейс под капотом]] — это пара `(itab/type, data)`? Reflection буквально работает с этой парой напрямую.
...
Классический кейс, зачем reflection реально нужен в проде — чтение struct tags (мы уже упоминали, что тег — часть подписи типа структуры, в [[Structs#Теги полей и анонимные структуры|Structs]]; то, на чём построены `encoding/json`, ORM-библиотеки и т.д.):
...
Это работает именно потому, что метод — просто функция с receiver-ом первым параметром (разбирали в теме про [[Interface#Методы — это просто функции с receiver-ом|интерфейсы]]) — `Call` по сути и есть вызов такой функции через reflection.
